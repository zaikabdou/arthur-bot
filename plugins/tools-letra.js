...
const styleNames = { 
    1: 'خط ناعم', 2: 'سيريف مزخرف كبير', 3: 'خط سميك', 4: 'حروف صغيرة', 5: 'خط مائل رياضي', 
    6: 'خط سميك رياضي', 7: 'خط كاليغرافي', 8: 'خط أحادي سميك', 9: 'خط عادي أحادي', 10: 'خط مائل سانس', 
    11: 'خط رياضي عادي', 12: 'خط كامل العرض', 13: 'حروف فوقية', 14: 'خط قوطي سميك', 
    15: 'حروف دائرية كبيرة', 16: 'حروف داخل مربعات', 17: 'خط كاليغرافي مائل', 18: 'حروف دائرية صغيرة', 
    19: 'خط كبير موحد', 20: 'خط فركتور', 21: 'حروف بين أقواس' 
}
...
async function handler(m, { text, command }) {
    switch (command) {
        case 'letra':
            if (!text) {
                return conn.reply(m.chat, '❀ من فضلك، أدخل النص الذي تريد زخرفته.', m)
            }
        ...
        case 'style':
            if (!text) {
                return conn.reply(m.chat, '❀ من فضلك، أدخل رقم الخط الجديد الذي تريد تعيينه <1|2|3>.', m)
            }
            let num = parseInt(text)
            if (!yStr[num]) {
                return conn.reply(m.chat, `ꕥ الخط ${num} غير متاح. استخدم من 1 إلى ${Object.keys(yStr).length}.`, m)
            }
            global.currentFontStyle = num
            let name = styleNames[num] || `${num}`
            let example = await global.style('مرحبا بالعالم', num)
            await m.react('✔️')
            conn.reply(m.chat, `❀ تم تغيير الخط إلى *(${name})*.\n> مثال: *${example}*`, m)
            break
    }
}
